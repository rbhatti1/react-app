pipeline {
    ...
    stages {
        ...
        stage('Build Docker Image') {
            ...
            steps {
                script {
                    app = docker.build("rbhatti1/react-app")
                    app.inside {
        ...
        stage('Push Docker Image') {
         ...
         stage('DeployToStaging') {
            ...
            steps {
                    script {
                        sh "docker pull rbhatti1/react-app:${env.BUILD_NUMBER}"
                        try {
                        ...
                        sh "docker run --restart always --name react-app -p 1233:80 -d rbhatti1/react-app:${env.BUILD_NUMBER}"
                    }
            }
        }
